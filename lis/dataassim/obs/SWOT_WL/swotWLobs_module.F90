...
!-----------------------------------------------------------------------------
  ...
  type, public :: swot_wl_dec
     ...
     integer,allocatable     :: reaches_data(:,:)
...
!EOP
    ...
    call ESMF_ConfigFindLabel(LIS_config,"SWOT water level distance map:", rc=status)
    do n=1,LIS_rc%nnest
       call ESMF_ConfigGetAttribute(LIS_config,wldistancemap,rc=status)
       call LIS_verify(status)
    enddo
    ...
    write(LIS_logunit,*)'[INFO] read SWOT water level data specifications'
    ...
    do n=1,LIS_rc%nnest
       ...
       allocate(swot_wl_struc(n)%reaches_data(LIS_rc%obs_lnc(k),LIS_rc%obs_lnr(k)))
       swot_wl_struc(n)%reaches_data = -9999

#if(defined USE_NETCDF3 || defined USE_NETCDF4)
       call LIS_verify(nf90_open(path=trim(wldistancemap),mode=NF90_NOWRITE,ncid=ftn),&
            '[ERR] Error opening file '//trim(wldistancemap))
       call LIS_verify(nf90_inq_varid(ftn,'lis_id',varid), '[ERR] Error with nf90_inq_varid: lis_id')
       call LIS_verify(nf90_get_var(ftn,varid, swot_wl_struc(n)%reaches_data, &
