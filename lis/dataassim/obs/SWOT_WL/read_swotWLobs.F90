...
!EOP
  ...
  if((time_now <= swot_wl_struc(n)%time_file+86400) .and. (mod(time_now, dt).eq.0)) then 
        ...
        do c =1,LIS_rc%obs_lnc(k)
           if(swot_wl_struc(n)%reaches_data(c,r).gt.0) then 
              do t=1, swot_wl_struc(n)%ntimes
                 if((swot_wl_struc(n)%time(swot_wl_struc(n)%reaches_data(c,r),t) >= time_start) &
                    .and. (swot_wl_struc(n)%time(swot_wl_struc(n)%reaches_data(c,r),t) <= time_end)) then
                    wlobs(c,r) = swot_wl_struc(n)%WLobs(swot_wl_struc(n)%reaches_data(c,r),t)

                    if(wlobs(c,r).gt.0) then !TODO: negtive wse value?? 
                       fnd = 1
                    endif 
                 endif
              enddo
     ...
     !TODO: !  Apply LSM-based QC of observations
